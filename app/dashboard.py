...
def classify_career_url(raw_url: str) -> dict[str, str | None]:
    ...
    elif host.endswith(".applicantpro.com"):
        source_type, source_key, site_kind = "applicantpro", host.split(".applicantpro.com", 1)[0], "ats_feed"
        canonical_path = "/" + "/".join(path_parts or ["jobs"])
    ...
    elif host.endswith("eightfold.ai") or host in {"portal.careers.hsbc.com", "jobs.ericsson.com", "apply.careers.microsoft.com"}:
    ...
    canonical_query = parsed.query if source_type in {"generic_html", "uber_jobs", "eightfold"} else ""
...
def jobs(
    ...
    return query(
        ...
        SELECT job.site_id, job.external_job_id, job.canonical_name, job.priority_tier,
               job.priority_score, job.priority_rank_in_tier, job.title,
               job.location, job.category, job.employment_type, job.role_status,
               job.canonical_role, job.role_stack, job.role_family,
               job.seniority_bucket, job.employment_bucket, job.location_bucket,
        ...
        LEFT JOIN jobpush.job_application_actions action
          ON action.site_id = job.site_id
         AND action.external_job_id = job.external_job_id
        WHERE {date_clause}
          AND {company_clause}
