...
WITH eligible AS (
    ...
    WHERE target.enabled
      ...
      AND site.source_type = 'rippling'
      AND site.normalized_domain = 'ats.rippling.com'
      AND NOT EXISTS (
          SELECT 1
...
SET verification_status = 'verified',
    ...
    END,
    reviewed_at = now(),
    reviewed_by = 'system:rippling-rank1-v1',
    review_notes = 'Auto-trusted rank-1 ats.rippling.com candidate after Rippling adapter was added; monitor crawl health',
    updated_at = now()
FROM eligible
...
WHERE EXISTS (
    ...
    WHERE site.consolidation_key = target.consolidation_key
      AND site.verification_status = 'verified'
      AND site.reviewed_by = 'system:rippling-rank1-v1'
);
