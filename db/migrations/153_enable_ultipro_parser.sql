...
-- Pilot enable five validated P2 UltiPro tenants after crawl_ultipro.py smoke tests.
WITH pilot_tenants(tenant_code) AS (
    VALUES
        ('YAS1000YAAMI'),
        ('WEL1014WELS'),
        ('VIS1004VIST'),
    ...
    JOIN pilot_tenants pilot
      ON upper(site.site_url) LIKE '%/' || pilot.tenant_code || '/%'
    ...
    ORDER BY site.consolidation_key, site.candidate_rank NULLS LAST, site.site_id
)
...
SET verification_status = 'verified',
    ...
    reviewed_at = now(),
    reviewed_by = 'system:ultipro-parser-pilot-v1',
    review_notes = concat_ws('; ', site.review_notes, 'Enabled UltiPro board after LoadSearchResults pilot validation'),
    updated_at = now()
FROM eligible
...
SELECT target.priority_tier, site.source_type, count(*) AS enabled_sites
FROM jobpush.career_sites site
JOIN jobpush.crawl_targets target USING (consolidation_key)
WHERE site.reviewed_by = 'system:ultipro-parser-pilot-v1'
GROUP BY 1, 2
ORDER BY 1, 2;
