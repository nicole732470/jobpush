...
BEGIN
    ...
    IF v_decision.classification_status IN ('target', 'non_target') THEN
        ...
        NEW.decision_reason := v_decision.decision_reason || ': candidate_profile 2026-06-27';
...
UPDATE jobpush.job_title_labels
SET decision_reason = 'profile_avoid_senior_sde_track: candidate_profile 2026-06-27',
    updated_at = now()
WHERE decision_reason = 'profile_avoid_senior_sde_track: candidate_profile 2026-06-24'
  AND COALESCE(rule_version, '') NOT LIKE 'manual%%';

UPDATE jobpush.job_title_label_history
SET decision_reason = 'profile_avoid_senior_sde_track: candidate_profile 2026-06-27'
WHERE decision_reason = 'profile_avoid_senior_sde_track: candidate_profile 2026-06-24'
  AND labeled_by = 'system:profile-title-rules-v2';
