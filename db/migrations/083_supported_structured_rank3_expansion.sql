...
-- Low-cost crawl expansion, 2026-06-27:
-- Enable rank-3 structured ATS candidates only when we already have a tested
-- adapter and the company has no verified site yet. This expands crawlable P1
-- coverage without spending Tavily credits and without trusting generic HTML.
WITH eligible AS (
    SELECT site.site_id,
...
SET verification_status = 'verified',
    ...
    END,
    reviewed_at = now(),
    reviewed_by = 'system:supported-structured-rank3-v1',
    review_notes = 'Auto-trusted supported structured ATS candidate, candidate_rank=3, no existing verified site; monitor crawl health',
    updated_at = now()
FROM eligible
...
WHERE EXISTS (
    ...
    WHERE site.consolidation_key = target.consolidation_key
      ...
      AND site.reviewed_by = 'system:supported-structured-rank3-v1'
);
