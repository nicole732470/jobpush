...
WHERE posting.active
  ...
  AND lower(posting.normalized_title) ~ '(^engineer$|(^|[^a-z])(ios (software )?(engineer|developer)|android (software )?(engineer|developer)|mobile (software )?(engineer|developer)|administrative assistant|admin assistant|project management|it operations management|cyber[ -]?security engineer)([^a-z]|$))'
GROUP BY 1,2,3,4
