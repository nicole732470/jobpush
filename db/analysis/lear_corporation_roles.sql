\echo '=== Lear Corporation consolidated rows ==='
...
WHERE canonical_name ILIKE '%lear%'
   OR consolidation_key ILIKE '%lear%'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Lear target SOC summary ==='
WITH lear_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%lear corporation%'
       OR consolidation_key ILIKE '%lear%'
    UNION
    SELECT fein FROM public.companies
    WHERE name ILIKE '%lear corporation%'
)
SELECT
...
JOIN lear_feins lf ON lf.fein = lcase.employer_fein
...
\echo '=== Lear target-role job_title (top by count) ==='
WITH lear_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%lear corporation%'
...
\echo '=== Lear all raw job_title counts (top 25, any SOC) ==='
WITH lear_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%lear corporation%'
