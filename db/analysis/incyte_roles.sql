...
WHERE canonical_name ILIKE '%incyte%'
   OR consolidation_key ILIKE '%incyte%'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Incyte target SOC summary ==='
WITH incyte_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%incyte%'
    UNION
    SELECT fein FROM public.companies
    WHERE name ILIKE '%incyte%'
)
SELECT
...
JOIN incyte_feins inf ON inf.fein = lcase.employer_fein
...
\echo '=== Incyte target-role job_title (top by count) ==='
WITH incyte_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%incyte%'
...
\echo '=== Incyte all raw job_title counts (top 25, any SOC) ==='
WITH incyte_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%incyte%'
