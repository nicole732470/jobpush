\echo '=== Agilent consolidated rows ==='
...
WHERE canonical_name ILIKE '%agilent%'
   OR consolidation_key ILIKE '%agilent%'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Agilent target SOC summary ==='
WITH ag_feins AS (
...
JOIN ag_feins af ON af.fein = lcase.employer_fein
...
\echo '=== Agilent target-role job_title (top by count) ==='
WITH ag_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%agilent%'
...
\echo '=== Agilent all raw job_title counts (top 25, any SOC) ==='
WITH ag_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%agilent%'
