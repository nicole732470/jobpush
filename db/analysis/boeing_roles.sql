...
WHERE canonical_name ILIKE '%boeing%'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Boeing target SOC summary ==='
WITH boeing_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%boeing%'
    UNION
    SELECT fein FROM public.companies WHERE name ILIKE '%boeing%'
)
SELECT
...
JOIN boeing_feins bf ON bf.fein = lcase.employer_fein
...
\echo '=== Boeing target-role job_title (top by count) ==='
WITH boeing_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%boeing%'
