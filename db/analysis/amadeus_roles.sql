\echo '=== Amadeus consolidated rows ==='
...
WHERE canonical_name ILIKE '%amadeus%'
   OR consolidation_key ILIKE '%amadeus%'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Amadeus target SOC summary ==='
WITH amadeus_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%amadeus%'
    UNION
    SELECT fein FROM public.companies
    WHERE name ILIKE '%amadeus%'
)
SELECT
...
JOIN amadeus_feins af ON af.fein = lcase.employer_fein
...
\echo '=== Amadeus target-role job_title (top by count) ==='
WITH amadeus_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%amadeus%'
...
\echo '=== Amadeus all raw job_title counts (top 25, any SOC) ==='
WITH amadeus_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%amadeus%'
