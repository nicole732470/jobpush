\echo '=== Coca-Cola consolidated rows ==='
...
WHERE canonical_name ILIKE '%coca-cola%'
   OR canonical_name ILIKE '%coca cola%'
   OR consolidation_key = 'the-coca-cola-company'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Coca-Cola target SOC summary ==='
WITH coca_cola_feins AS (
...
JOIN coca_cola_feins cf ON cf.fein = lcase.employer_fein
...
\echo '=== Coca-Cola target-role job_title (top by count) ==='
WITH coca_cola_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%coca-cola%'
...
\echo '=== Coca-Cola all raw job_title counts (top 25, any SOC) ==='
WITH coca_cola_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%coca-cola%'
