...
WHERE canonical_name ILIKE '%pacific gas%'
   OR canonical_name ILIKE '%PG&E%'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== PG&E target SOC summary ==='
WITH pge_feins AS (
    ...
    WHERE name ILIKE '%pacific gas%electric%'
)
SELECT
...
JOIN pge_feins pf ON pf.fein = lcase.employer_fein
...
\echo '=== PG&E target-role job_title (top by count) ==='
WITH pge_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%pacific gas%'
...
\echo '=== PG&E all raw job_title counts (top 25, any SOC) ==='
WITH pge_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%pacific gas%'
