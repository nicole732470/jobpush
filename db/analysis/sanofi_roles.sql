...
WHERE canonical_name ILIKE '%sanofi%'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Sanofi target SOC summary ==='
WITH sanofi_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%sanofi%'
    UNION
    SELECT fein FROM public.companies WHERE name ILIKE '%sanofi%'
)
SELECT
...
JOIN sanofi_feins sf ON sf.fein = lcase.employer_fein
...
\echo '=== Sanofi target-role job_title (top by count) ==='
WITH sanofi_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%sanofi%'
