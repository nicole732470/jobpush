\echo '=== Kyndryl consolidated rows ==='
...
WHERE canonical_name ILIKE '%kyndryl%'
   OR consolidation_key = 'kyndryl'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Kyndryl target SOC summary ==='
WITH kyndryl_feins AS (
...
JOIN kyndryl_feins kf ON kf.fein = lcase.employer_fein
...
\echo '=== Kyndryl target-role job_title (top by count) ==='
WITH kyndryl_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%kyndryl%'
...
\echo '=== Kyndryl all raw job_title counts (top 25, any SOC) ==='
WITH kyndryl_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%kyndryl%'
