\echo '=== Baker Hughes consolidated rows ==='
...
WHERE canonical_name ILIKE '%baker hughes%'
   OR consolidation_key ILIKE '%baker-hughes%'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Baker Hughes target SOC summary ==='
WITH bh_feins AS (
...
JOIN bh_feins bf ON bf.fein = lcase.employer_fein
...
\echo '=== Baker Hughes target-role job_title (top by count) ==='
WITH bh_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%baker hughes%'
...
\echo '=== Baker Hughes all raw job_title counts (top 25, any SOC) ==='
WITH bh_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%baker hughes%'
