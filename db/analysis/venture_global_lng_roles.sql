\echo '=== Venture Global LNG consolidated rows ==='
...
WHERE canonical_name ILIKE '%venture global%'
   OR consolidation_key ILIKE '%venture-global%'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Venture Global target SOC summary ==='
WITH vg_feins AS (
    ...
    WHERE name ILIKE '%venture global%lng%'
       OR name ILIKE '%venture global lng%'
)
SELECT
...
JOIN vg_feins vf ON vf.fein = lcase.employer_fein
...
\echo '=== Venture Global target-role job_title ==='
WITH vg_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%venture global%'
...
\echo '=== Venture Global all raw job_title counts (any SOC) ==='
WITH vg_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%venture global%'
