\echo '=== Texas Instruments consolidated rows ==='
...
WHERE canonical_name ILIKE '%texas instruments%'
   OR consolidation_key ILIKE '%texas-instruments%'
ORDER BY lca_count DESC
LIMIT 10;

\echo '=== Texas Instruments target SOC summary ==='
WITH ti_feins AS (
    ...
    WHERE name ILIKE '%texas instruments%'
)
SELECT
...
JOIN ti_feins tf ON tf.fein = lcase.employer_fein
...
\echo '=== Texas Instruments target-role job_title (top by count) ==='
WITH ti_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%texas instruments%'
...
\echo '=== Texas Instruments all raw job_title counts (top 25, any SOC) ==='
WITH ti_feins AS (
    SELECT unnest(member_feins) AS fein
    FROM jobpush.company_targets_consolidated
    WHERE canonical_name ILIKE '%texas instruments%'
