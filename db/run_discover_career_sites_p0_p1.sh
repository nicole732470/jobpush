...
LIMIT="${DISCOVERY_LIMIT:-600}"
[[ "$LIMIT" =~ ^[1-9][0-9]*$ ]] || { echo "DISCOVERY_LIMIT must be a positive integer" >&2; exit 2; }

RUN_ID="career-p0-p1-$(date -u +%Y%m%dT%H%M%SZ)-$$"
COHORT="effective-tier-p0-p1-score-expansion"
WORK_DIR="$(mktemp -d -t jobpush-career-p0-p1.XXXXXX)"
...
"${PSQL[@]}" -c "\copy (
  ...
  ORDER BY CASE target.priority_tier WHEN 'P0' THEN 0 WHEN 'P1' THEN 1 END,
...
if [[ "$TARGET_COUNT" -le 0 ]]; then
  echo "No never-searched P0/P1 targets found."
  exit 0
fi
echo "Searching $TARGET_COUNT P0/P1 companies; expected basic-search credits: $TARGET_COUNT"
