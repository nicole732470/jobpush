...
psql -h "$RDS_HOST" -U "$RDS_USER" -d "$RDS_DB" -v ON_ERROR_STOP=1 \
  -f "$SCRIPT_DIR/migrations/008_priority_score_components.sql"
psql -h "$RDS_HOST" -U "$RDS_USER" -d "$RDS_DB" -v ON_ERROR_STOP=1 \
  -f "$SCRIPT_DIR/migrations/009_product_role_title_fn.sql"
psql -h "$RDS_HOST" -U "$RDS_USER" -d "$RDS_DB" -v ON_ERROR_STOP=1 \
  -f "$SCRIPT_DIR/refresh/refresh_company_targets.sql"

psql -h "$RDS_HOST" -U "$RDS_USER" -d "$RDS_DB" -P pager=off -c \
  "SELECT priority_score, COUNT(*) FROM jobpush.company_targets GROUP BY 1 ORDER BY 1;"

psql -h "$RDS_HOST" -U "$RDS_USER" -d "$RDS_DB" -P pager=off -c \
  "SELECT target_role_score, lca_count_score, chicago_score, COUNT(*)
