...
RUN_ID="career-p3-$(date -u +%Y%m%dT%H%M%SZ)-$$"
COHORT="p3-score-desc-expansion"
WORK_DIR="$(mktemp -d -t jobpush-career-p3.XXXXXX)"
...
if [[ "$TARGET_COUNT" -le 0 ]]; then
  echo "No never-searched P3 targets found."
  exit 0
fi
echo "Searching $TARGET_COUNT P3 companies by priority_score DESC; expected basic-search credits: $TARGET_COUNT"
